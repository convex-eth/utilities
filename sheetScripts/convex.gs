...
function convexLockedCVX(address) {
...
function convexAPRWithPrice(poolName, crvPrice, cvxPrice) {
    ...
    if (pool.extras != undefined && pool.extras.length > 0) {
        for (var i in pool.extras) {
            ...
            Logger.log("extra per year: " +perYear +"  price: " +price +" apr: " +(perYear*price) );
            apr += (perYear * price);
        }
    }
...
function lockedBalanceOf(address, contract, decimals) {
    var lockedBalanceOfHex = "59355736";
    address = padHex(address);
    var url = "https://api.etherscan.io/api?module=proxy&action=eth_call&to=" + contract + "&data=0x" + lockedBalanceOfHex + address + "&tag=latest&apikey=" + ETHERSCAN_KEY;
    var response = UrlFetchApp.fetch(url);
    var json = response.getContentText();
    var data = JSON.parse(json);
    // var hexValue = data.result;
...
function boostedSupply(contract) {
    var boostedSupply = "75aadf61";
    var url = "https://api.etherscan.io/api?module=proxy&action=eth_call&to=" + contract + "&data=0x" + boostedSupply + "&tag=latest&apikey=" + ETHERSCAN_KEY;
    var response = UrlFetchApp.fetch(url);
    var json = response.getContentText();
    var data = JSON.parse(json);
    var hexValue = data.result;
...
function rewardRateOf(contract, token) {
    var rewardData = "48e5d9f8";
    var tokenPad = padHex(token);
    var url = "https://api.etherscan.io/api?module=proxy&action=eth_call&to=" + contract + "&data=0x" + rewardData +tokenPad + "&tag=latest&apikey=" + ETHERSCAN_KEY;
    var response = UrlFetchApp.fetch(url);
    var json = response.getContentText();
